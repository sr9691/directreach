...
final class Reading_Room_Controller extends WP_REST_Controller
{
        ...
        try {
            ...
            if ($counts['problem'] > 0) {
                ...
                $progress_count = $wpdb->get_var($wpdb->prepare(
                    ...
                    AND transitioned_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)"
                ));
    ...
    /**
     * Permission check for all RTR endpoints.
     *
     * Accepts either:
     *   1. WordPress logged-in user with edit_posts (admin UI)
     *   2. X-API-Key matching cpd_api_key (primary RTR key)
     *   3. X-API-Key matching directreach_journeyos_api_key (JourneyOS shared key)
     */
    public function check_permission(WP_REST_Request $request = null): bool|WP_Error {
        ...
        if ($request) {
            ...
            if (!empty($api_key)) {
                ...
                // Also accept the JourneyOS shared key
